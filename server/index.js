...
app.post('/api/render', async (req, res) => {
    try {
        // --- CHANGE START ---
        // Extract canvasSettings along with layers
        const { layers, canvasSettings } = req.body;

        if (!Array.isArray(layers)) {
            return res.status(400).json({ error: 'Invalid payload: "layers" must be an array.' });
        }

        // Pass canvasSettings to the worker job
        const job = await renderQueue.add('render-job', { 
            layers,
            canvasSettings 
        });
        // --- CHANGE END ---
...
app.get('/api/jobs/:id', async (req, res) => {
    ...
    try {
        ...
        if (state === 'failed') return res.json({ status: 'failed', error: job.failedReason });
