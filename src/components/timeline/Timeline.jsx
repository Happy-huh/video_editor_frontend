...
import { Play, Pause, Magnet, AlignLeft, Video, Music, Type, Subtitles, Lock, Unlock, VolumeX, Volume2, Scissors } from 'lucide-react';
...
export const Timeline = ({ height }) => {
    const {
        ...
        toggleTrackLock, toggleTrackMute, splitLayer,
    ...
    return (
        <div style={{...styles.container, height}} onMouseMove={handleMouseMove} onMouseUp={() => setInteraction(null)} onMouseLeave={() => setInteraction(null)}>
             <div style={styles.toolbar}>
                 ...
                 <div style={{width:1, height:20, background:'#333', margin:'0 8px'}}></div>
                 <button onClick={() => selectedLayerId && splitLayer(selectedLayerId, currentTime)} title="Split Clip (S)" style={{color: selectedLayerId ? '#fff' : '#444', background:'none', border:'none', cursor:'pointer'}}><Scissors size={16}/></button>
